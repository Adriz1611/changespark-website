    ...
    >
      ...
      <div className="flex flex-col items-center justify-center text-center gap-10">
        {team_member.map((team, key) => (
          ...
          >
            <h1 className="font-heading text-4xl font-bold my-10 uppercase">
              {team.heading}
            </h1>
            <div className="flex flex-col md:flex-row items-center justify-center gap-20 ">
              {team.people.map((person, key) => (
                <motion.div
                  ...
                  key={key}
                  className="flex-col flex items-center text-center rounded-lg w-80 py-16 px-8 bg-background-200"
                >
                  <Image
                    className="rounded-full mb-20 aspect-square object-cover"
                    width={150}
                    height={150}
                    src={person.img}
                    alt={person.name}
                  />
                  <h2 className="font-heading font-semibold text-2xl text-background-800">
                    {person.name}
                  </h2>
                  <p className="font-paragraph uppercase mt-3 font-medium text-accent-700">
                    {person.role}
                  </p>
                  <p className="font-paragraph font-medium text-background-800 mt-3">
                    {person.desc}
                  </p>
                </motion.div>
              ))}
