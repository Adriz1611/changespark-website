...
export default function YuvaVikasPage() {
  return (
    <main className="overflow-hidden bg-white font-paragraph text-gray-700">
      <section className="relative min-h-[740px] flex items-center bg-gradient-to-br from-emerald-950 via-green-900 to-teal-800 text-white">
        <div className="absolute inset-0 opacity-20" style={{ backgroundImage: "linear-gradient(rgba(255,255,255,.13) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,.13) 1px, transparent 1px)", backgroundSize: "48px 48px" }} />
        <div className="absolute -top-24 -right-24 h-96 w-96 rounded-full bg-yellow-300/20 blur-3xl" />
        <div className="absolute -bottom-20 -left-20 h-[32rem] w-[32rem] rounded-full bg-emerald-400/20 blur-3xl" />
        <div className="container-custom relative pt-28 pb-20 lg:pt-36 lg:pb-28">
          <motion.div initial={{ opacity: 0, y: 25 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="max-w-4xl">
            <div className="inline-flex items-center gap-2 rounded-full border border-white/25 bg-white/10 px-4 py-2 text-sm font-medium backdrop-blur-sm"><Sparkles className="h-4 w-4 text-yellow-300" /> Youth as Change Makers</div>
            ...
            <a href="#journey" className="btn-primary mt-10 inline-flex items-center gap-2">Start your journey <ArrowRight className="h-5 w-5" /></a>
          </motion.div>
        </div>
      </section>
      ...
      <section id="journey" className="section-padding bg-gray-50"><div className="container-custom"><SectionTitle eyebrow="Your journey" title="Discover. Learn. Participate. Lead. Impact." text="Mishan Yuva Vikas is designed as a journey—not just a program." /><div className="relative grid gap-5 md:grid-cols-5">{journey.map(([title, text, Icon], index) => <FadeIn key={title} className="relative"><div className="h-full rounded-2xl bg-white p-6 shadow-sm ring-1 ring-gray-100"><span className="flex h-11 w-11 items-center justify-center rounded-xl bg-green-100 font-bold text-green-800">{index + 1}</span><Icon className="mt-6 h-7 w-7 text-green-700" /><h3 className="mt-4 text-xl font-bold text-gray-900 font-heading">{title}</h3><p className="mt-3 text-sm leading-relaxed">{text}</p></div></FadeIn>)}</div><div className="mt-10 text-center"><p className="mb-5 text-lg font-medium text-gray-800">Your journey can begin today.</p><Link href="/contact-us" className="btn-primary inline-flex items-center gap-2">Start your journey <ArrowRight className="h-5 w-5" /></Link></div></div></section>
